...
Page({
    ...
    onShow: function () {
      ...
      var sign = wx.getStorageSync("sign");
      wx.request({
        url: 'https://shop.playonwechat.com/api/mine?sign=' + sign + '&operator_id=' + app.data.kid,
        success:function(res){
          console.log("mine",res);
          if(res.data.status){
            var mine = res.data.data;
            var list = that.data.list; //my各订单展示数目
            var is_membership = mine.is_membership;
    ...
    erwei: function () {
      var sign = wx.getStorageSync("sign");
      wx.request({
        url: 'https://shop.playonwechat.com/api/create-qrcode?sign=' + sign + '&operator_id=' + app.data.kid,
        success: function (res) {
          console.log("二维码", res);
          if (res.data.status) {
